export type Form1040Facts = {
line1zSources: Array<TaxFactSource>;
line2aSources: Array<TaxFactSource>;
line2bSources: Array<TaxFactSource>;
line3aSources: Array<TaxFactSource>;
line3bSources: Array<TaxFactSource>;
line4aSources: Array<TaxFactSource>;
line4bSources: Array<TaxFactSource>;
line5aSources: Array<TaxFactSource>;
line5bSources: Array<TaxFactSource>;
line6aSources: Array<TaxFactSource>;
line6bSources: Array<TaxFactSource>;
line7Sources: Array<TaxFactSource>;
line8Sources: Array<TaxFactSource>;
line10Sources: Array<TaxFactSource>;
line12Sources: Array<TaxFactSource>;
line13Sources: Array<TaxFactSource>;
line16Sources: Array<TaxFactSource>;
line17Sources: Array<TaxFactSource>;
line20Sources: Array<TaxFactSource>;
line23Sources: Array<TaxFactSource>;
line25aSources: Array<TaxFactSource>;
line25bSources: Array<TaxFactSource>;
line25cSources: Array<TaxFactSource>;
line26Sources: Array<TaxFactSource>;
line31Sources: Array<TaxFactSource>;
filingStatus: string;
line1z: number;
line2a: number;
line2b: number;
line3a: number;
line3b: number;
line4a: number;
line4b: number;
line5a: number;
line5b: number;
line6a: number;
line6b: number;
line7: number;
line8: number;
line9: number;
line10: number;
line11: number;
line12Source: string;
line12: number;
line13: number;
line14: number;
line15: number;
line16TaxComputation: string;
line16: number;
line17: number;
line18: number;
line19: number;
line20: number;
line21: number;
line22: number;
line23: number;
line24: number;
line25a: number;
line25b: number;
line25c: number;
line25d: number;
line26: number;
line31: number;
line32: number;
line33: number;
line34: number;
line35a: number;
line36: number;
line37: number;
line38: number;
};
export type Form1116Facts = {
passiveIncomeSources: Array<TaxFactSource>;
generalIncomeSources: Array<TaxFactSource>;
foreignTaxSources: Array<TaxFactSource>;
line4bSources: Array<TaxFactSource>;
sourcedByPartnerElectionSources: Array<TaxFactSource>;
totalPassiveIncome: number;
totalGeneralIncome: number;
totalForeignTaxes: number;
totalLine4b: number;
netForeignSourceTaxableIncome: number;
totalSourcedByPartnerIncome: number;
creditValue: number;
deductionValueAtThirtySevenPercent: number;
recommendation: string | null;
totalK1Box5: number;
turboTaxAlert: boolean;
hasUserOverride: boolean;
};
export type Form4797Facts = {
partISources: Array<TaxFactSource>;
partIISources: Array<TaxFactSource>;
partIIISources: Array<TaxFactSource>;
schedule1Sources: Array<TaxFactSource>;
scheduleDSources: Array<TaxFactSource>;
partINet1231: number;
partIIOrdinary: number;
partIIIRecapture: number;
netToSchedule1Line4: number;
netToScheduleDLongTerm: number;
hasActivity: boolean;
};
export type Form4952AmtFacts = {
line1to3InvestmentInterest: number;
line4aGrossInvestmentIncome: number;
line4bQualifiedDividends: number;
line4cAfterQualifiedDividends: number;
line4dNetGainFromDisposition: number;
line4eNetCapitalGainFromDisposition: number;
line4fNetShortTermFromDisposition: number;
line4gElected: number;
line4hTotalInvestmentIncome: number;
line5InvestmentExpenses: number;
line6NetInvestmentIncome: number;
line7DisallowedCarryforward: number;
line8DeductibleInvestmentInterest: number;
line2cAdjustment: number;
};
export type Form4952CalculationRow = {
label: string;
amount: number;
role: string;
note: string | null;
};
export type Form4952CarryDestination = {
sources: Array<TaxFactSource>;
destination: string;
label: string;
formLine: string;
grossInterest: number;
allowedDeduction: number;
carryforward: number;
share: number;
citation: string;
};
export type Form4952Facts = {
investmentInterestSources: Array<TaxFactSource>;
investmentExpenseSources: Array<TaxFactSource>;
excludedInvestmentExpenseSources: Array<TaxFactSource>;
materialParticipationScheduleEInterestSources: Array<TaxFactSource>;
grossInvestmentIncomeFromK1Sources: Array<TaxFactSource>;
qualifiedDividendSources: Array<TaxFactSource>;
carryDestinations: Array<Form4952CarryDestination>;
tracingSplitSources: Array<Form4952TracingSplit>;
line4aCalculationRows: Array<Form4952CalculationRow>;
line4cCalculationRows: Array<Form4952CalculationRow>;
line4dCalculationRows: Array<Form4952CalculationRow>;
line4eCalculationRows: Array<Form4952CalculationRow>;
totalInvestmentInterestExpense: number;
totalInvestmentExpenses: number;
totalExcludedInvestmentExpenses: number;
totalMaterialParticipationScheduleEInterest: number;
grossInvestmentIncomeFromScheduleB: number;
grossInvestmentIncomeFromK1: number;
grossInvestmentIncomeTotal: number;
line4cNetInvestmentIncomeAfterQualifiedDividends: number;
netInvestmentIncomeBeforeQualifiedDividendElection: number;
totalQualifiedDividends: number;
deductibleInvestmentInterestExpense: number;
disallowedCarryforward: number;
deductibleScheduleEAboveLine: number;
deductibleScheduleAItemized: number;
carryforwardScheduleE: number;
carryforwardScheduleA: number;
allocationMethod: string;
allocationMethodDescription: string;
line4dNetGainFromDisposition: number;
line4eNetCapitalGainFromDisposition: number;
line4fNetShortTermFromDisposition: number;
line4gElectedQualifiedDividendsAndGain: number;
line4hTotalInvestmentIncome: number;
line5InvestmentExpenses: number;
line5TcjaSuspended: boolean;
line5SuspensionReason: string;
line6NetInvestmentIncome: number;
electionNiiWithoutElection: number;
electionExcessInvestmentInterest: number;
electionAvailableForElection: number;
electionMaxBeneficial: number;
recommendedElection: number;
line18AllowedDeduction: number;
line19aScheduleEPassthru: number;
line20ScheduleAItemized: number;
amt: Form4952AmtFacts | null;
};
export type Form4952TracingSplit = {
sourceId: string;
label: string;
grossInterest: number;
scheduleAInterest: number;
scheduleEInterest: number;
scheduleAShare: number;
scheduleEShare: number;
taxDocumentId: number | null;
formType: string | null;
box: string | null;
code: string | null;
};
export type Form6251Facts = {
sourceEntries: Array<Form6251SourceEntryFact>;
manualReviewReasons: Array<string>;
line1TaxableIncome: number;
line2aTaxesOrStandardDeduction: number;
line2aSource: string;
line2cInvestmentInterest: number;
line2dDepletion: number;
line2kDispositionOfProperty: number;
line2lPost1986Depreciation: number;
line2mPassiveActivities: number;
line2nLossLimitations: number;
line2tIntangibleDrillingCosts: number;
line3OtherAdjustments: number;
adjustmentTotal: number;
amti: number;
exemption: number;
exemptionBase: number;
exemptionReduction: number;
exemptionPhaseoutThreshold: number;
amtTaxBase: number;
amtRateSplitThreshold: number;
amtBeforeForeignCredit: number;
line8AmtForeignTaxCredit: number;
tentativeMinTax: number;
regularTax: number;
regularForeignTaxCredit: number;
regularTaxAfterCredits: number;
amt: number;
filingStatus: string;
requiresStatementReview: boolean;
};
export type Form6251SourceEntryFact = {
label: string;
code: string;
line: string;
amount: number;
description: string;
requiresStatementReview: boolean;
};
export type Form6781Facts = {
shortTermSources: Array<TaxFactSource>;
longTermSources: Array<TaxFactSource>;
shortTermTotal: number;
longTermTotal: number;
netGain: number;
};
export type Form8582ActivityFact = {
activityName: string;
ein: string | null;
isRentalRealEstate: boolean;
activeParticipation: boolean;
currentIncome: number;
currentLoss: number;
priorYearUnallowed: number;
overallGainOrLoss: number;
allowedLossThisYear: number;
suspendedLossCarryforward: number;
};
export type Form8582Facts = {
activities: Array<Form8582ActivityFact>;
totalPassiveIncome: number;
totalPassiveLoss: number;
totalPriorYearUnallowed: number;
netPassiveResult: number;
rentalAllowance: number;
totalAllowedLoss: number;
totalSuspendedLoss: number;
netDeductionToReturn: number;
isLossLimited: boolean;
magi: number;
isMarried: boolean;
realEstateProfessional: boolean;
};
export type Form8606Facts = {
conversions: Array<Form8606SourceRowFact>;
distributions: Array<Form8606SourceRowFact>;
line1_nondeductibleContributions: number;
line2_priorYearBasis: number;
line3_totalBasis: number;
line6_yearEndFmv: number;
line7_distributionsNotConverted: number;
line8_convertedToRoth: number;
line9_total: number;
line10_proRataRatio: number;
line11_basisInConversion: number;
line12_basisInDistributions: number;
line13_totalBasisUsed: number;
line14_basisCarriedForward: number;
line15c_taxableDistributions: number;
line18_taxableConversions: number;
taxableToForm1040Line4b: number;
hasActivity: boolean;
};
export type Form8606SourceRowFact = {
payerName: string;
grossDistribution: number;
taxableAmount: number;
distributionCode: string;
isIra: boolean;
};
export type Form8829EntityFact = {
homeOfficeLines: Array<Form8829LineFact>;
line36Sources: Array<TaxFactSource>;
line43Sources: Array<TaxFactSource>;
line44Sources: Array<TaxFactSource>;
entityId: number | null;
entityName: string;
method: string;
officeSqft: number | null;
homeSqft: number | null;
monthsUsed: number;
businessUsePercentage: number;
priorYearOpCarryover: number;
priorYearOpCarryoverCa: number;
priorYearDepreciationCarryover: number;
priorYearDepreciationCarryoverCa: number;
line1OfficeSqft: number;
line2HomeSqft: number;
line3BusinessUsePercentage: number;
line7BusinessUsePercentage: number;
line8TentativeProfit: number;
line14DeductibleMortgageInterestAndTaxes: number;
line15OperatingExpenseLimit: number;
line23OperatingExpensesTotal: number;
line24AllowableOperatingIndirectExpenses: number;
line25PriorYearOpCarryover: number;
line26TotalOperatingExpenseClaim: number;
line27AllowableOperatingExpenses: number;
line28ExcessCasualtyAndDepreciationLimit: number;
line30Depreciation: number;
line31PriorYearExcessCasualtyAndDepreciationCarryover: number;
line32TotalExcessCasualtyAndDepreciation: number;
line33AllowableExcessCasualtyAndDepreciation: number;
line36AllowableHomeOfficeDeduction: number;
line43OperatingCarryoverToNextYear: number;
line43OperatingCarryoverToNextYearCa: number;
line44ExcessCasualtyAndDepreciationCarryoverToNextYear: number;
line44ExcessCasualtyAndDepreciationCarryoverToNextYearCa: number;
carryoverToNextYear: number;
carryoverToNextYearCa: number;
regularDeduction: number;
simplifiedDeduction: number;
limitationReason: string;
};
export type Form8829Facts = {
entities: Array<Form8829EntityFact>;
line36AllowableHomeOfficeDeductionTotal: number;
line43OperatingCarryoverToNextYearTotal: number;
line43OperatingCarryoverToNextYearCaTotal: number;
line44ExcessCasualtyAndDepreciationCarryoverToNextYearTotal: number;
line44ExcessCasualtyAndDepreciationCarryoverToNextYearCaTotal: number;
carryoverToNextYearTotal: number;
carryoverToNextYearCaTotal: number;
};
export type Form8829LineFact = {
sources: Array<TaxFactSource>;
lineRef: string;
label: string;
directExpense: number;
indirectExpense: number;
allowable: number;
};
export type Form8949Facts = {
rows: Array<Form8949RowFact>;
scheduleDRollups: Array<ScheduleDRollupFact>;
washSaleAdjustments: Array<WashSaleAdjustmentFact>;
reportingMode: string;
rowCount: number;
washSaleAdjustmentCount: number;
washSaleAdjustmentTotal: number;
};
export type Form8949RowFact = {
form8949Box: string | null;
description: string;
dateAcquired: string | null;
dateSold: string;
proceeds: number;
costBasis: number;
adjustmentCode: string | null;
adjustmentAmount: number;
gainOrLoss: number;
isShortTerm: boolean;
isCovered: boolean | null;
isSummaryRow: boolean;
accountName: string | null;
taxDocumentId: number | null;
sourceTransactionId: string | null;
};
export type Form8959Facts = {
wageSources: Array<TaxFactSource>;
withholdingSources: Array<TaxFactSource>;
wages: number;
threshold: number;
excessWages: number;
additionalTax: number;
medicareTaxWithheld: number;
regularMedicareTaxWithholding: number;
additionalMedicareWithholding: number;
};
export type Form8960Facts = {
componentSources: Array<TaxFactSource>;
taxableInterest: number;
ordinaryDividends: number;
netCapGains: number;
passiveIncome: number;
nonpassiveTradingIncome: number;
investmentInterestExpense: number;
stateLocalForeignIncomeTax: number;
grossNII: number;
totalDeductions: number;
netInvestmentIncome: number;
magi: number;
thresholdSingle: number;
thresholdMarriedFilingJointly: number;
magiExcessSingle: number;
magiExcessMarriedFilingJointly: number;
niitTaxSingle: number;
niitTaxMarriedFilingJointly: number;
needsMagi: boolean;
};
export type Form8995AEntityFact = {
sources: Array<TaxFactSource>;
entityKey: string;
label: string;
sourceKind: string;
isSstb: boolean;
qbiIncome: number;
qbiLossNettingAdjustment: number;
qbiAfterLossNetting: number;
applicablePercentage: number;
adjustedQbi: number;
w2Wages: number;
ubia: number;
w2WageLimit: number;
w2WageUbiaLimit: number;
wageUbiaLimit: number;
qbiComponentBeforeLimit: number;
wageUbiaLimitedQbiComponent: number;
phaseInReduction: number;
qualifiedBusinessIncomeComponent: number;
};
export type Form8995AFacts = {
entities: Array<Form8995AEntityFact>;
threshold: number;
phaseInRange: number;
phaseInPercentage: number;
totalQualifiedBusinessIncomeComponent: number;
qualifiedReitPtpComponent: number;
deductionBeforeIncomeLimit: number;
taxableIncomeBeforeQbi: number;
netCapitalGain: number;
taxableIncomeLessNetCapitalGain: number;
incomeLimitation: number;
deduction: number;
};
export type Form8995EntityFact = {
sources: Array<TaxFactSource>;
entityKey: string;
label: string;
sourceKind: string;
qbiIncome: number;
reitDividends: number;
ptpIncome: number;
qbiComponent: number;
w2Wages: number;
ubia: number;
isSstb: boolean;
sectionNotes: string | null;
needsForm8995AReview: boolean;
form8995AReviewReason: string | null;
};
export type Form8995Facts = {
entities: Array<Form8995EntityFact>;
line1Sources: Array<TaxFactSource>;
line6Sources: Array<TaxFactSource>;
reviewSources: Array<TaxFactSource>;
totalQbi: number;
totalQbiComponent: number;
qualifiedReitDividends: number;
qualifiedPtpIncome: number;
reitPtpComponent: number;
taxableIncomeBeforeQbi: number;
netCapitalGain: number;
taxableIncomeLessNetCapitalGain: number;
taxableIncomeCap: number;
deduction: number;
thresholdSingle: number;
thresholdMarriedFilingJointly: number;
aboveThreshold: boolean;
form8995A: Form8995AFacts | null;
};
export type PartnershipBasisEventFact = {
id: number;
taxYear: number;
eventType: string;
basisSide: string;
amount: number;
sourceType: string;
taxDocumentId: number | null;
taxDocumentAccountId: number | null;
accountId: number | null;
k1Box: string | null;
k1Code: string | null;
sourcePath: string | null;
sourceLabel: string | null;
reviewStatus: string;
};
export type PartnershipBasisFacts = {
interests: Array<PartnershipBasisInterestFacts>;
year: number;
};
export type PartnershipBasisInterestFacts = {
events: Array<PartnershipBasisEventFact>;
interestId: number;
partnershipName: string;
partnershipEin: string | null;
accountId: number | null;
taxYear: number;
beginningTaxBasisCapital: number;
endingTaxBasisCapital: number;
beginningBookCapital: number;
endingBookCapital: number;
insideBasisConfidence: string;
reviewStatus: string;
isStale: boolean;
worksheet: PartnershipBasisWorksheetFacts;
};
export type PartnershipBasisWorksheetFacts = {
beginningOutsideBasis: number;
capitalContributions: number;
taxableIncomeIncrease: number;
taxExemptIncomeIncrease: number;
liabilityIncrease: number;
cashDistributions: number;
propertyDistributionsBasis: number;
liabilityDecrease: number;
deductionsLossesDecrease: number;
nondeductibleExpensesDecrease: number;
foreignTaxesDecrease: number;
distributionGain: number;
suspendedLossCarryforward: number;
endingOutsideBasis: number;
liquidationGainLoss: number | null;
};
export type QuarterTotals = {
q1: number;
q2: number;
q3: number;
q4: number;
};
export type Schedule1Facts = {
line1aSources: Array<TaxFactSource>;
line2aSources: Array<TaxFactSource>;
line3Sources: Array<TaxFactSource>;
line4Sources: Array<TaxFactSource>;
line5Sources: Array<TaxFactSource>;
line6Sources: Array<TaxFactSource>;
line7Sources: Array<TaxFactSource>;
line8zSources: Array<TaxFactSource>;
line8Sources: Array<TaxFactSource>;
line8bSources: Array<TaxFactSource>;
line8hSources: Array<TaxFactSource>;
line8iSources: Array<TaxFactSource>;
line15Sources: Array<TaxFactSource>;
line1aTotal: number;
line2aTotal: number;
line3Total: number;
line4Total: number;
line5Total: number;
line6Total: number;
line7Total: number;
line8bTotal: number;
line8hTotal: number;
line8iTotal: number;
line8zTotal: number;
line9TotalOtherIncome: number;
line15Total: number;
};
export type Schedule3Facts = {
line1Sources: Array<TaxFactSource>;
line2Sources: Array<TaxFactSource>;
line3Sources: Array<TaxFactSource>;
line4Sources: Array<TaxFactSource>;
line5aSources: Array<TaxFactSource>;
line5bSources: Array<TaxFactSource>;
line6Sources: Array<TaxFactSource>;
line9Sources: Array<TaxFactSource>;
line10Sources: Array<TaxFactSource>;
line11Sources: Array<TaxFactSource>;
line12Sources: Array<TaxFactSource>;
line13Sources: Array<TaxFactSource>;
line1ForeignTaxCredit: number;
line2ChildDependentCareCredit: number;
line3EducationCredits: number;
line4RetirementSavingsCredit: number;
line5aResidentialCleanEnergyCredit: number;
line5bEnergyEfficientHomeImprovementCredit: number;
line7OtherNonrefundableCredits: number;
line8TotalNonrefundableCredits: number;
line9NetPremiumTaxCredit: number;
line10ExtensionPayment: number;
line11ExcessSocialSecurityWithheld: number;
line12FuelTaxCredit: number;
line14OtherPaymentsRefundableCredits: number;
line15TotalPaymentsRefundableCredits: number;
};
export type ScheduleAFacts = {
stateIncomeTaxSources: Array<TaxFactSource>;
salesTaxSources: Array<TaxFactSource>;
realEstateTaxSources: Array<TaxFactSource>;
personalPropertyTaxSources: Array<TaxFactSource>;
mortgageInterestSources: Array<TaxFactSource>;
investmentInterestSources: Array<TaxFactSource>;
charitableCashSources: Array<TaxFactSource>;
charitableNoncashSources: Array<TaxFactSource>;
otherItemizedSources: Array<TaxFactSource>;
otherItemizedTransactions: Array<TaxPreviewTransaction>;
stateIncomeTaxTotal: number;
salesTaxTotal: number;
selectedLine5aType: string;
selectedLine5aTotal: number;
realEstateTaxTotal: number;
personalPropertyTaxTotal: number;
saltPaidBeforeCap: number;
saltCap: number;
saltDeduction: number;
saltCapMagi: number | null;
saltCapUsesEstimatedMagi: boolean;
saltCapNeedsMagi: boolean;
mortgageInterestTotal: number;
grossInvestmentInterestTotal: number;
investmentInterestTotal: number;
disallowedInvestmentInterest: number;
totalInterest: number;
charitableCashTotal: number;
charitableNoncashTotal: number;
charitableTotal: number;
otherItemizedTotal: number;
totalItemizedDeductions: number;
standardDeductionSingle: number;
standardDeductionMarriedFilingJointly: number;
shouldItemizeSingle: boolean;
shouldItemizeMarriedFilingJointly: boolean;
};
export type ScheduleBFacts = {
interestSources: Array<TaxFactSource>;
ordinaryDividendSources: Array<TaxFactSource>;
qualifiedDividendSources: Array<TaxFactSource>;
directInterestTotal: number;
k1InterestTotal: number;
interestTotal: number;
directOrdinaryDividendTotal: number;
k1OrdinaryDividendTotal: number;
ordinaryDividendTotal: number;
qualifiedDividendTotal: number;
form4952Line5aTotal: number;
};
export type ScheduleCEntityFact = {
grossReceiptSources: Array<TaxFactSource>;
returnsAndAllowancesSources: Array<TaxFactSource>;
expenseSources: Array<TaxFactSource>;
homeOfficeSources: Array<TaxFactSource>;
flaggedExpenseRows: Array<ScheduleCFlaggedExpenseRowFact>;
entityId: number | null;
entityName: string;
grossReceipts: number;
returnsAndAllowances: number;
grossIncomeAfterReturns: number;
expenses: number;
expensesBeforeHomeOffice: number;
homeOfficeClaimed: number;
homeOfficeAllowable: number;
homeOfficeDisallowed: number;
homeOfficePriorCarryforward: number;
homeOfficeCarryoverToNextYear: number;
homeOfficeLimitationReason: string;
tentativeProfitBeforeHomeOffice: number;
netProfitBeforeHomeOffice: number;
netProfit: number;
};
export type ScheduleCFacts = {
entities: Array<ScheduleCEntityFact>;
line31Sources: Array<TaxFactSource>;
grossReceiptsTotal: number;
returnsAndAllowancesTotal: number;
grossIncomeAfterReturns: number;
expensesTotal: number;
expensesBeforeHomeOffice: number;
tentativeProfitBeforeHomeOffice: number;
homeOfficeAllowable: number;
homeOfficeDisallowed: number;
homeOfficePriorCarryforward: number;
homeOfficeCarryoverToNextYear: number;
netProfit: number;
netProfitCumulativeByQuarter: QuarterTotals;
netProfitRoutedToSchedule1: number;
};
export type ScheduleCFlaggedExpenseRowFact = {
transactionId: number;
date: string;
description: string | null;
amount: number;
accountId: number | null;
taxCharacteristic: string;
label: string;
category: string;
reason: string;
};
export type ScheduleDFacts = {
form8949Rollups: Array<ScheduleDRollupFact>;
line5Sources: Array<TaxFactSource>;
line3Sources: Array<TaxFactSource>;
line4Sources: Array<TaxFactSource>;
line10Sources: Array<TaxFactSource>;
line11Sources: Array<TaxFactSource>;
line12Sources: Array<TaxFactSource>;
line13Sources: Array<TaxFactSource>;
ambiguous11SSources: Array<TaxFactSource>;
line1aGainLoss: number;
line1bGainLoss: number;
line2GainLoss: number;
line3GainLoss: number;
line4GainLoss: number;
line5GainLoss: number;
line6Carryover: number;
line7NetShortTerm: number;
line8aGainLoss: number;
line8bGainLoss: number;
line9GainLoss: number;
line10GainLoss: number;
line11GainLoss: number;
line12GainLoss: number;
line13CapitalGainDistributions: number;
line14Carryover: number;
line15NetLongTerm: number;
line16Combined: number;
line21LimitedLossOrGain: number;
appliedToReturn: number;
carryforward: number;
totalBusinessCapGains: number;
totalPersonalCapGains: number;
limitedBusinessCapGains: number;
limitedPersonalCapGains: number;
ambiguous11SAmount: number;
};
export type ScheduleDRollupFact = {
form8949Box: string;
isShortTerm: boolean;
scheduleDLine: string;
totalProceeds: number;
totalCostBasis: number;
totalAdjustment: number;
netGainOrLoss: number;
rowCount: number;
};
export type ScheduleEFacts = {
miscIncomeSources: Array<TaxFactSource>;
box1Sources: Array<TaxFactSource>;
box2Sources: Array<TaxFactSource>;
box3Sources: Array<TaxFactSource>;
box4Sources: Array<TaxFactSource>;
box11ZZSources: Array<TaxFactSource>;
box13ZZSources: Array<TaxFactSource>;
traderNiiSources: Array<TaxFactSource>;
form4952InvestmentInterestSources: Array<TaxFactSource>;
materialParticipationTraderInterestSources: Array<TaxFactSource>;
miscIncomeTotal: number;
totalBox1: number;
totalBox2: number;
totalBox3: number;
totalBox4: number;
totalBox5: number;
totalBox11ZZ: number;
totalBox13ZZ: number;
totalTraderNii: number;
totalForm4952InvestmentInterest: number;
totalMaterialParticipationTraderInterest: number;
totalPassive: number;
totalNonpassive: number;
totalNonpassiveIncome: number;
totalNonpassiveLoss: number;
grandTotal: number;
};
export type ScheduleFFacts = {
grossIncomeSources: Array<TaxFactSource>;
expenseSources: Array<TaxFactSource>;
line34Sources: Array<TaxFactSource>;
grossFarmIncome: number;
totalFarmExpenses: number;
netFarmProfit: number;
hasActivity: boolean;
};
export type ScheduleSEFacts = {
entries: Array<TaxFactSource>;
wageSources: Array<TaxFactSource>;
medicareTaxWithheldSources: Array<TaxFactSource>;
scheduleFSources: Array<TaxFactSource>;
netEarningsFromSE: number;
seTaxableEarnings: number;
socialSecurityWageBase: number;
socialSecurityWages: number;
remainingSocialSecurityWageBase: number;
socialSecurityTaxableEarnings: number;
socialSecurityTax: number;
medicareWages: number;
medicareTaxWithheld: number;
medicareTaxableEarnings: number;
medicareTax: number;
additionalMedicareThreshold: number;
additionalMedicareTaxableEarnings: number;
additionalMedicareTax: number;
seTax: number;
deductibleSeTax: number;
};
export type TaxFactSource = {
sourceType: string;
routing: string | null;
id: string;
label: string;
amount: number;
taxDocumentId: number | null;
taxDocumentAccountId: number | null;
accountId: number | null;
formType: string | null;
box: string | null;
code: string | null;
routingReason: string | null;
notes: string | null;
isReviewed: boolean;
reviewStatus: string;
reviewAction: string | null;
};
export type TaxPreviewFacts = {
year: number;
scheduleC: ScheduleCFacts;
form8829: Form8829Facts;
scheduleF: ScheduleFFacts;
scheduleSE: ScheduleSEFacts;
form8959: Form8959Facts;
schedule1: Schedule1Facts;
schedule3: Schedule3Facts;
scheduleB: ScheduleBFacts;
form4952: Form4952Facts;
scheduleA: ScheduleAFacts;
scheduleE: ScheduleEFacts;
form6781: Form6781Facts;
scheduleD: ScheduleDFacts;
form8949: Form8949Facts;
form4797: Form4797Facts;
form8606: Form8606Facts;
form1116: Form1116Facts;
form8960: Form8960Facts;
form8995: Form8995Facts;
form6251: Form6251Facts;
form8582: Form8582Facts;
form1040: Form1040Facts;
partnershipBasis: PartnershipBasisFacts;
};
export type TaxPreviewTransaction = {
transactionId: number;
date: string;
description: string | null;
amount: number;
accountId: number | null;
};
export type WashSaleAdjustmentFact = {
id: string;
lossSaleId: string;
replacementPurchaseId: string;
symbol: string;
saleDate: string;
replacementDate: string;
disallowedLoss: number;
saleAccountId: number | null;
saleAccountName: string | null;
replacementAccountId: number | null;
replacementAccountName: string | null;
isCrossAccount: boolean;
reason: string;
saleLotId: number | null;
replacementLotId: number | null;
detectionNote: string;
};
