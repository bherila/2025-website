...
describe('SavedJobPicker', () => {
  ...
  it('hides the picker behind a login prompt for guests and does not fetch', () => {
    render(<SavedJobPicker inputs={DEFAULT_OPPORTUNITY_COST_INPUTS} authenticated={false} onApply={jest.fn()} />)
  ...
  it('shows an empty state when the user has no saved jobs', async () => {
    ...
    render(<SavedJobPicker inputs={DEFAULT_OPPORTUNITY_COST_INPUTS} authenticated onApply={jest.fn()} />)
  ...
  it('loads a chosen saved job into the current slot', async () => {
    ...
    render(<SavedJobPicker inputs={DEFAULT_OPPORTUNITY_COST_INPUTS} authenticated onApply={onApply} />)
