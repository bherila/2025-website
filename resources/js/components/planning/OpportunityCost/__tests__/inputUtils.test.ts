...
describe('normalizeOpportunityCostInputs (backend-shaped inputs)', () => {
  it('keeps a current job whose nullable backend fields (liquidityDate, grantValue) are null', () => {
    // Mirrors the shape emitted by OpportunityCostInputs::defaults() on the PHP side, where
    ...
    const normalized = normalizeOpportunityCostInputs(serverInputs)
  ...
  it('coerces an empty optional liquidityDate to null and drops grants with no grant date', () => {
    ...
    const normalized = normalizeOpportunityCostInputs(inputs)
