...
import type { JobSpec, OpportunityCostInputs, OptionGrant, RsuGrant } from './types'

export type OpportunityCostFormSectionId = 'basics' | 'current-job' | 'offers'

export interface OpportunityCostFormSectionMeta {
  id: OpportunityCostFormSectionId
  label: string
  shortLabel: string
  description: string
  icon: LucideIcon
  presentation: 'column'
  component: MillerRegistryEntry<unknown, OpportunityCostFormSectionId>['component']
  meta: {
    description: string
    icon: LucideIcon
  }
...
interface OpportunityCostFormProps {
  inputs: OpportunityCostInputs
  onChange: (inputs: OpportunityCostInputs) => void
}

interface OpportunityCostFormSectionProps extends OpportunityCostFormProps {
  section: OpportunityCostFormSectionId
}
...
export function notRenderedViaMillerShell(): never {
  throw new Error('OpportunityCost does not render via MillerRegistryShell')
}

export const OPPORTUNITY_COST_FORM_SECTIONS: OpportunityCostFormSectionMeta[] = [
  {
    id: 'basics',
...
function updateJob(inputs: OpportunityCostInputs, jobId: string, updater: (job: JobSpec) => JobSpec): OpportunityCostInputs {
...
export function OpportunityCostFormSection({ inputs, section, onChange }: OpportunityCostFormSectionProps): ReactElement {
  if (section === 'basics') {
    return (
      <Card>
        <CardHeader>
...
export function OpportunityCostForm(props: OpportunityCostFormProps): ReactElement {
  return (
    <div className="space-y-4">
      {OPPORTUNITY_COST_FORM_SECTIONS.map((section) => (
        <OpportunityCostFormSection key={section.id} {...props} section={section.id} />
      ))}
    </div>
  )
}
