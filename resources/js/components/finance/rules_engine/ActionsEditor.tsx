...
function getFieldConfig(type: string): ActionFieldConfig {
  switch (type) {
    case 'add_tag':
    case 'remove_tag':
      return { showTarget: true, showPayload: false, useTagSelect: true }
    case 'find_replace':
      return {
        targetLabel: 'Search',
  ...
  return (
    <div className="space-y-3">
      ...
      {actions.map((action, index) => {
        ...
        return (
          <div key={index} className="flex flex-col gap-3 rounded-md border border-border/40 p-3">
            <div className="flex flex-wrap items-end gap-2">
              ...
              {config.showTarget && !config.stackFields && (
                <div className="min-w-[140px] flex-1">
                  {config.useTagSelect ? (
                    <>
                      ...
                      <TagSelect
                        value={action.target}
                        onChange={(v) => updateAction(index, { target: v })}
                        tags={tags}
              ...
              {config.showPayload && !config.stackFields && (
                <div className="min-w-[140px] flex-1">
