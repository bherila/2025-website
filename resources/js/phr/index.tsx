...
interface ImagingPanelProps {
  selectedPatient: PhrPatient | null
  studies: PhrDicomStudy[]
  onUploadComplete: (upload: PhrDicomUpload) => void
  onRefresh: () => void
  setBusy: (busy: boolean) => void
}
...
function PHRApp() {
  ...
  const [dicomStudies, setDicomStudies] = useState<PhrDicomStudy[]>([])
  ...
  async function loadPatientRecords(patientId: number): Promise<void> {
    ...
    try {
      const [rawLabs, rawVitals, rawDicomStudies] = await Promise.all([
      ...
      setDicomStudies(PhrDicomStudiesResponseSchema.parse(rawDicomStudies).studies)
    } catch (caught) {
      setError(errorMessage(caught))
    } finally {
      setBusy(false)
      ...
      <div className="grid gap-5 lg:grid-cols-[320px_minmax(0,1fr)]">
        ...
        <main className="flex min-w-0 flex-col gap-5">
          ...
          <ImagingPanel
            ...
            onUploadComplete={() => selectedPatientId !== null && void loadPatientRecords(selectedPatientId)}
            onRefresh={() => selectedPatientId !== null && void loadPatientRecords(selectedPatientId)}
            setBusy={setBusy}
          />
          <SharingPanel
            selectedPatient={selectedPatient}
...
function ImagingPanel({ selectedPatient, studies, onUploadComplete, onRefresh, setBusy }: ImagingPanelProps) {
  ...
  async function upload(event: FormEvent<HTMLFormElement>): Promise<void> {
    ...
    if (!selectedPatient || acceptedFiles.length === 0) {
      setError('Select a DICOM directory or files to upload.')
      return
    }
    ...
    try {
      ...
      onUploadComplete(response.upload)
    } catch (caught) {
      setError(errorMessage(caught))
    } finally {
      setBusy(false)
