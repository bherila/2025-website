...
<div class="mb-6 flex justify-between items-center">
    ...
    <div class="flex gap-3 items-center">
        <button data-action="reload" class="px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-md hover:bg-gray-50 inline-flex items-center gap-2">
            <i data-lucide="refresh-cw" class="w-4 h-4" aria-hidden="true"></i>
            Refresh
        </button>
        <form method="GET" class="inline-block">
            <select id="period-select" name="period" class="px-4 py-2 text-sm font-medium border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500">
...
<!-- Chart.js -->
<script @cspNonce src="https://cdn.jsdelivr.net/npm/chart.js@4.4.0/dist/chart.umd.min.js"></script>
<script @cspNonce>
document.addEventListener('DOMContentLoaded', function() {
    // Wire up reload buttons (replaces onclick="location.reload()")
    document.querySelectorAll('[data-action="reload"]').forEach(function(btn) {
        btn.addEventListener('click', function() { location.reload(); });
    });

    // Wire up period select (replaces onchange="this.form.submit()")
    var periodSelect = document.getElementById('period-select');
    if (periodSelect) {
        periodSelect.addEventListener('change', function() { this.form.submit(); });
    }
