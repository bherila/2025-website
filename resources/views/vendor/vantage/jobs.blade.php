...
<div class="mb-8">
    <div class="flex items-center justify-between">
        ...
        <div class="flex items-center space-x-3">
            ...
            <button data-action="reload" class="inline-flex items-center px-3 py-2 border border-gray-300 shadow-sm text-sm leading-4 font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
...
document.addEventListener('DOMContentLoaded', function() {
    // Wire up reload button (replaces onclick="location.reload()")
    document.querySelectorAll('[data-action="reload"]').forEach(function(btn) {
        btn.addEventListener('click', function() { location.reload(); });
    });

    // Event delegation for tag filter buttons (replaces onclick="addTagToFilter(...)")
    document.addEventListener('click', function(e) {
        var btn = e.target.closest('.tag-filter-button');
        if (!btn) return;
        ...
        var currentTags = tagsInput.value.trim();
        var normalizedTag = String(tag).toLowerCase().trim();
        var newTagsValue;
        if (currentTags === '') {
            newTagsValue = normalizedTag;
        } else {
            var tags = currentTags.split(',').map(function(t) { return t.trim().toLowerCase(); }).filter(function(t) { return t; });
            if (!tags.includes(normalizedTag)) {
                newTagsValue = currentTags + ', ' + normalizedTag;
            } else {
                newTagsValue = tags.filter(function(t) { return t !== normalizedTag; }).join(', ');
            }
        }
    ...
    // Ctrl/Cmd + K keyboard shortcut to focus tags input
