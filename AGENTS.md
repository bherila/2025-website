...
- Tax Preview fact calculation lives in per-slice builders under `app/Services/Finance/TaxPreviewFacts/Builders/`; keep `TaxPreviewFactsService` as orchestration glue when adding new forms or slices.
...
9. **PII in PRs, Issues, Commits**: Never write identifying real-world names into PR titles/bodies, issue titles/bodies, commit messages, or branch names. This includes client/company names, personal names, email addresses, broker account numbers, fund/issuer names, and any other identifier that could leak who or what the data is about. Use **database `table_name#id` references** instead — e.g. `client_companies#7`, `client_agreements#6`, `client_invoices#66`, `users#7`, `fin_tax_documents#19`. IDs are safe because they are meaningless without DB access. Apply this to follow-up data-cleanup issues, reconciliation discrepancy notes, and bug reports that reference real records. If you need a slug, hourly rate, or signer name, write "(resolve from `client_companies.slug` for `id=X`)" or "(signer name on record)" rather than embedding the value. The same rule applies to AI-agent issue bodies generated from prod data: round-trip the data through `table#id` references before posting. Why: this is a personal-project repo whose issues/PRs are public; once committed, the only way to remove PII is rewriting history per-revision.
...
- Laravel 13 supports limiting eagerly loaded records natively, without external packages: `$query->latest()->limit(10);`.
