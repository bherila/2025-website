...
| `scheduleA` | Schedule A lines 5a/5b/5c/7/8a/9/11/12/16/17 | W-2 Box 17 state tax, user-entered itemized deductions, K-1 Box 13L portfolio deductions, and Form 4952 deductible investment interest are exposed with source metadata. The builder applies the SALT cap and returns both single and MFJ standard-deduction comparisons because filing status is not yet backend-owned by this fact slice. |
