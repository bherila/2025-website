...
- When an active-loop passenger boards, the rest of the loop shifts one slot forward so the visible front-of-loop passenger is always the next-in-queue to board. The trailing slot becomes empty and is refilled by the feeder front when it reaches the feeder join. Shifted passengers interpolate from their previous slot to their new one in **offset space** — the tween samples the loop curve live each frame as the loop rotates, rather than animating toward a world point frozen at the rebuild moment. Freezing a world point made the passenger jerk backward to a stale spot and then race forward; sampling live keeps the advance a smooth forward step.
- Passengers from the active feeder walk into the back of the loop along a curved bezier path that joins tangentially. The walk-in is timed to *complete* exactly as the empty loop slot reaches the feeder join, so the passenger merges gracefully into the gap as it arrives rather than chasing a slot that has already advanced (which would cross over the passengers in between). An in-progress walk-in is preserved across scene rebuilds so it never snaps back to the feeder mid-step.
...
- Boarding should be tolerant around the gate, but matching must stay visually anchored *to* the gate — a passenger should not board while still in the upper half of the loop. The post-gate boarding window is kept tight (just under one queue-slot, enough to absorb frame-to-frame travel). The loop-shift case (a boarding bumps a trailing passenger forward by a slot, possibly across the gate) is handled by seeding that passenger's gate cycle from its *pre-shift* offset, so the bump registers as a genuine gate crossing on the next frame instead of being erased — no wide window, and no forced extra lap.
