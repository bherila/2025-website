...
class EquityCompensationFactsBuilder extends TaxPreviewFactBuilder
{
            ...
            if ($row['type'] === 'iso') {
                ...
                if ($bargainElement !== 0.0) {
                    $source = $this->optionSource(
                        ...
                        routingReason: 'ISO bargain element is an AMT adjustment routed to Form 6251 line 3 for the opportunity-cost projection.',
                    );
                    $sources[] = $source;
                    $form6251SourceEntries[] = new Form6251SourceEntryFact(
                        ...
                        line: '3',
                        amount: $bargainElement,
                        description: 'ISO bargain element from opportunity-cost option exercise',
                    );
                }
            } else {
                ...
                if ($bargainElement !== 0.0) {
                    $sources[] = $this->optionSource(
                        ...
                        routingReason: 'NSO bargain element is ordinary compensation income for the opportunity-cost projection.',
                    );
                }
            }
    ...
    private function optionSource(JobSpec $job, array $row, int $year, float $amount, TaxFactSourceType $sourceType, TaxFactRouting $routing, string $routingReason): TaxFactSource
    {
        return new TaxFactSource(
            ...
            formType: 'opportunity_cost_projection',
            routing: $routing,
            routingReason: $routingReason,
            notes: 'Generated by the opportunity-cost equity-compensation tax facts builder.',
        );
    }
    ...
    private function equitySaleSource(JobSpec $job, int $year, float $amount): TaxFactSource
    {
        return new TaxFactSource(
            ...
            routingReason: 'Opportunity-cost liquid equity proceeds are routed as Schedule D review inputs until basis and holding period are known.',
