...
class ComputeCareerCompRequest extends FormRequest
{
    ...
    private static function jobRules(string $prefix, bool $nullable): array
    {
        ...
        return [
            ...
            "{$prefix}.rsuGrants.*.sourceAwardId" => ['nullable', 'string', 'max:120'],
            "{$prefix}.rsuGrants.*.sourceAwardRowIds" => ['nullable', 'array', 'max:240'],
            "{$prefix}.rsuGrants.*.sourceAwardRowIds.*" => ['integer', 'min:1'],
            "{$prefix}.rsuGrants.*.symbol" => ['nullable', 'string', 'max:16'],
            "{$prefix}.rsuGrants.*.rsuSource" => ['nullable', 'array'],
            "{$prefix}.rsuGrants.*.rsuSource.mode" => ['required_with:'.$prefix.'.rsuGrants.*.rsuSource', Rule::in(['snapshot'])],
            "{$prefix}.rsuGrants.*.rsuSource.capturedAt" => ['required_with:'.$prefix.'.rsuGrants.*.rsuSource', 'string', 'max:64'],
            "{$prefix}.rsuGrants.*.rsuSource.source" => ['required_with:'.$prefix.'.rsuGrants.*.rsuSource', Rule::in(['fin_equity_awards'])],
            "{$prefix}.rsuGrants.*.rsuSource.sourceAwardRowIds" => ['required_with:'.$prefix.'.rsuGrants.*.rsuSource', 'array', 'max:240'],
            "{$prefix}.rsuGrants.*.rsuSource.sourceAwardRowIds.*" => ['integer', 'min:1'],
            "{$prefix}.rsuGrants.*.rsuSource.sourceHash" => ['required_with:'.$prefix.'.rsuGrants.*.rsuSource', 'string', 'max:128'],
