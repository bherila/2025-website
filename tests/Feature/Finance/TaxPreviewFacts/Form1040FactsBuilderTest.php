...
class Form1040FactsBuilderTest extends TestCase
{
    ...
    public function test_income_and_adjustment_lines_round_constituents_before_derived_line_math(): void
    {
        $facts = $this->buildFacts(
            interest: 10.49,
            ordinaryDividends: 0.49,
            adjustments: 0.51,
        );

        $this->assertSame(10.0, $facts->line9);
        $this->assertSame(0.51, $facts->line10);
        $this->assertSame(9.0, $facts->line11);
    }

    public function test_line14_rounds_each_constituent_before_summing_into_whole_dollar_total(): void
    {
        $facts = $this->buildFacts(
            standardDeductionSingle: 45073.62,
            qbiDeduction: 184.74,
        );

        $this->assertSame(45073.62, $facts->line12);
        $this->assertSame(184.74, $facts->line13);
        $this->assertSame(45259.0, $facts->line14);
    }

    private function buildFacts(
        float $standardDeductionSingle = 0.0,
        float $qbiDeduction = 0.0,
        float $interest = 0.0,
        float $ordinaryDividends = 0.0,
        float $adjustments = 0.0,
    ): Form1040Facts {
        return $this->builder()->build(
            w2Docs: [],
            docs1099: [],
            scheduleB: $this->scheduleB(interest: $interest, ordinaryDividends: $ordinaryDividends),
            schedule1: $this->schedule1(adjustments: $adjustments),
            scheduleA: $this->scheduleA(standardDeductionSingle: $standardDeductionSingle),
            scheduleD: $this->scheduleD(),
            schedule3: $this->schedule3(),
            scheduleSE: $this->scheduleSE(),
            form8959: $this->form8959(),
            form8995: $this->form8995(deduction: $qbiDeduction),
    ...
    private function schedule1(float $adjustments = 0.0): Schedule1Facts
    {
