...
class PalCarryforwardControllerTest extends TestCase
{
    ...
    {
        $this->getJson('/api/finance/pal-carryforwards?year=2025')
            ->assertOk()
            ->assertExactJson([]);
    }
    ...
    {
        $this->postJson('/api/finance/pal-carryforwards', [
            'tax_year' => 2025,
    ...
    public function test_index_scopes_to_year(): void
    {
        ...
        $response = $this->getJson('/api/finance/pal-carryforwards?year=2025');
        $response->assertOk();
    ...
    {
        $create = $this->postJson('/api/finance/pal-carryforwards', [
            'tax_year' => 2025,
            'activity_name' => 'Fund A',
            'ordinary_carryover' => -1000,
        ]);
        ...
        $this->putJson("/api/finance/pal-carryforwards/{$id}", ['ordinary_carryover' => -2500])
            ->assertOk()
    ...
    public function test_destroy_removes_carryforward(): void
    {
        ...
        $this->deleteJson("/api/finance/pal-carryforwards/{$id}")->assertOk();
    ...
    public function test_cannot_modify_another_users_carryforward(): void
    {
        ...
        $this->actingAs($this->user);
        $this->putJson("/api/finance/pal-carryforwards/{$id}", ['ordinary_carryover' => -9999])->assertNotFound();
        $this->deleteJson("/api/finance/pal-carryforwards/{$id}")->assertNotFound();
    }
    ...
    {
        $response = $this->postJson('/api/finance/pal-carryforwards', [
            'tax_year' => 2025,
            'activity_name' => 'Fund A',
