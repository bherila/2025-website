...
class ClientInvoiceTest extends TestCase
{
    ...
    public function test_generate_all_preserves_outgoing_monthly_catch_up_for_gap_before_successor_start(): void
    {
        ...
        $successor = ClientAgreement::factory()->for($this->company)->create([
            ...
            'monthly_retainer_hours' => 20,
            'hourly_rate' => 250.00,
            'active_date' => Carbon::parse('2025-06-01'),
            'termination_date' => null,
            'rollover_months' => 3,
            'is_visible_to_client' => true,
        ]);

        $aprilEntry = ClientTimeEntry::create([
            'client_company_id' => $this->company->id,
            'user_id' => $this->admin->id,
            'project_id' => $this->project->id,
            'date_worked' => '2025-04-10',
        ...
        $mayEntry = ClientTimeEntry::create([
            'client_company_id' => $this->company->id,
            'user_id' => $this->admin->id,
            'project_id' => $this->project->id,
            'date_worked' => '2025-05-10',
            'minutes_worked' => 60,
            'name' => 'Successor first work period',
            'is_billable' => true,
        ]);
        ...
        $this->assertNotNull($outgoingAprilInvoice, 'Outgoing agreement should still bill true gap-month work.');

        $successorMayInvoice = ClientInvoice::query()
            ->where('client_agreement_id', $successor->id)
            ->whereDate('period_start', '2025-05-01')
            ->first();

        $this->assertNotNull($successorMayInvoice, 'Successor agreement should bill its first monthly work period.');

        $aprilEntry->refresh();
        $mayEntry->refresh();

        $this->assertSame($outgoingAprilInvoice->client_invoice_id, $aprilEntry->invoiceLine->client_invoice_id);
        $this->assertSame($successorMayInvoice->client_invoice_id, $mayEntry->invoiceLine->client_invoice_id);
    }
