...
- **Invoice Line Types**: `prior_month_retainer` ($0, work covered by retainer/rollover/negative balance pool), `additional_hours` (not typically used in give-and-take but available for manual billing), `prior_month_billable` (not typically used in current model), `retainer` (monthly fee), `expense` (reimbursable costs), `credit` (informational balance updates), `adjustment` (manual adjustments).
